...
saveRDS(sample.combined, file="IntegrateSample_Tothers.rds")
...
pdf("combineplot_To.pdf")
CombinePlots(plots = plots,ncol = 4,legend = 'right')
dev.off()

pdf("DimCluster1_To.pdf")
DimPlot(object = nCoV.integrated, reduction = 'tsne',label = TRUE)
dev.off()
